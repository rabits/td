...
{
    setCamera(cam);
    setStyle(USER_CS_FREELOOK);
    setControlledObject(this);
...
bool CUser::useKeyboard()
{
    // If user used keyboard - then returned true
    return true;
}

bool CUser::useMouse()
{
    // If user used mouse - then returned true
    return true;
}

bool CUser::useJoyStick()
{
    // If user used joystick - then returned true
    return true;
}
...
-----------------------------------------------------------------------------*/
void CUser::setCamera(Ogre::Camera* cam)
{
    m_pCamera = cam;
}

Ogre::Camera* CUser::getCamera()
{
    return m_pCamera;
}
...
-----------------------------------------------------------------------------*/
void CUser::setTarget(Ogre::SceneNode* target)
{
    if( target != m_pTarget )
    {
        m_pTarget = target;
...
Ogre::SceneNode* CUser::getTarget()
{
    return m_pTarget;
}
...
-----------------------------------------------------------------------------*/
void CUser::setYawPitchDist(Ogre::Radian yaw, Ogre::Radian pitch, Ogre::Real dist)
{
...
-----------------------------------------------------------------------------*/
void CUser::setTopSpeed(Ogre::Real topSpeed)
{
    m_topSpeed = topSpeed;
}

Ogre::Real CUser::getTopSpeed()
{
    return m_topSpeed;
}
...
-----------------------------------------------------------------------------*/
void CUser::setStyle(CameraStyle style)
{
    if( m_style != USER_CS_ORBIT && style == USER_CS_ORBIT )
    {
    ...
    else if( m_style != USER_CS_FREELOOK && style == USER_CS_FREELOOK )
    {
        m_pCamera->setAutoTracking(false);
        m_pCamera->setFixedYawAxis(true);
    }
    else if( m_style != USER_CS_MANUAL && style == USER_CS_MANUAL )
    {
        m_pCamera->setAutoTracking(false);
        manualStop();
    }
...
CUser::CameraStyle CUser::getStyle()
{
    return m_style;
}
